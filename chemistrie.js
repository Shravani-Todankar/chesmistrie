...
(function () {
  ...
  if (window.ScrollTrigger) {
    gsap.from(".product", {
      opacity: 0, y: 60, duration: 1, ease: "power2.out", stagger: 0.08,
      scrollTrigger: { trigger: ".shop__grid", start: "top 75%" },
    });
    gsap.from(".shop__head > *", {
  ...
  /* ───── Active Index — featured + list interaction ───── */
  (function initActives() {
    const list = $("#activeList");
    const feature = $("#activeFeature");
    if (!list || !feature) return;
    const numEl = feature.querySelector(".actives__feature-num strong");
    const tierEl = feature.querySelector(".actives__feature-tier");
    const doseEl = feature.querySelector(".actives__feature-dose");
    const nameEl = feature.querySelector(".actives__feature-name");
    const descEl = feature.querySelector(".actives__feature-desc");
    const specs = feature.querySelectorAll(".actives__feature-specs dd");
    /* specs order: shelf, source, found */

    function selectRow(row) {
      $$(".actives__row", list).forEach(r => r.classList.remove("is-selected"));
      row.classList.add("is-selected");
      const d = row.dataset;
      if (numEl) numEl.textContent = d.num;
      if (tierEl) tierEl.textContent = d.tier === "hero" ? "— Hero active" : "— Supporting active";
      if (doseEl) doseEl.textContent = d.dose;
      if (nameEl) nameEl.textContent = d.name;
      if (descEl) descEl.textContent = d.desc;
      if (specs[0]) specs[0].textContent = d.shelf;
      if (specs[1]) specs[1].textContent = d.source;
      if (specs[2]) specs[2].textContent = d.found;
    }

    list.addEventListener("click", (e) => {
      const row = e.target.closest(".actives__row");
      if (row) selectRow(row);
    });

    /* filter chips */
    $$(".actives__chip").forEach((chip) => {
      chip.addEventListener("click", () => {
        $$(".actives__chip").forEach(c => c.classList.remove("is-active"));
        chip.classList.add("is-active");
        const f = chip.dataset.filter;
        $$(".actives__row", list).forEach((row) => {
          if (f === "all" || row.dataset.tier === f) row.classList.remove("is-hidden");
